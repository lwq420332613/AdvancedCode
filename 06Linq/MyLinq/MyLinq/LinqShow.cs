        ...
        public void Show()
        {
            ...
            { //常规情况下数据过滤
                ...
                }
                {
                    ...
                    var result = list.ElevenWhere<Student>(s => s.Age < 30);
                }
                {
                    new List<int>() { 1, 23, 4, 5, 436, 4, 65, 6757, 56, 7, 767 }.ElevenWhere<int>(i => i > 10);
                    //new List<int>() .AsQueryable().Where()
                        }

                //ExtendMethod.ElevenWhere(studentList, s => s.Age < 30);
                //var listExtend = studentList.ElevenWhere<Student>(s => s.Age < 30);

                //foreach (var item in listExtend)//延迟到使用数据的才去获取   用的才去查，用多少查多少 DataReader
                //                                //推迟一切可以推迟的，直到用的时候才去查找
                //{
                //    //return;
                //    //return 
                //}
            ...
            }
            {
                ...
                //ExtendMethod.ElevenWhere(studentList, s => s.Age > 30);
                //studentList.ElevenWhere(s => s.Age > 30);
            }
            {
                //new List<int>() { 1, 23, 4, 5, 436, 4, 65, 6757, 56, 7, 767 }.ElevenWhere<int>(i => i < 100);
            }
            {
